...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.send_button = CTkButton(self, text ='>', width=50, height=40, command = self.send_message, fg_color= "purple")
        self.send_button.place(x=0, y=0)

        self.img_button = CTkButton(self, text='📷', width=50, height=40, command=self.open_img, fg_color= "purple")
        self.img_button.place(x=0, y=0)

        self.adaptive_ui()

        self.username = "ДамскийУгодник217"
        try:
            self.sock = socket(AF_INET, SOCK_STREAM)
            self.sock.connect(('0.tcp.ngrok.io', 19342))
            hello = f"TEXT@{self.username}@[SYSTEM] {self.username} приднався(-лась) до чату!\n"
            self.sock.send(hello.encode())
            threading.Thread(target=self.recv_message, daemon=True).start()
            self.add_message("Вы вошли в сеть")
        except:
            self.add_message(f"Сегодня не заходишь. {Exception}")
    ...
    def toggle_show_menu(self):
        if self.is_show_menu:
            ...
            self.entry.pack()
            # Кнопка збереження
            self.save_button = CTkButton(self.menu_frame, text="Зберегти", command=self.save_name, fg_color= "purple")
    ...
    def adaptive_ui(self):
        ...
        self.img_button.place(x=self.winfo_width() - self.img_button.winfo_width() * 2,
                                   y=self.winfo_height() - self.send_button.winfo_height())
    ...
    def add_message(self, message, img = None):
            message_frame = CTkFrame(self. chat_field, fg_color= '#DE45D1')
            ...
            if img:
                ...
                CTkLabel(message_frame, text=message, wraplength=wrapleng_size,
                         text_color='white', compound='top', justify='left').pack(pady=5)
    # Оновлення ніку
    def save_name(self):
        new_name = self.entry.get().strip()
        if new_name:
            self.username = new_name
            self.add_message(f"Вы себя переименовали, теперь Вас зовут {self.username}.")
        else:
            self.add_message(f"Вы не ввели новое имя.")
    # Відправка повідомленнь
    def send_message(self):
        message = self.message_entry.get().strip()
        if message:
            self.add_message(f"Ваше сообщение: {message}")
            # Формат, хто , що
            data = f"TEXT@{self.username}@{message}+\n"
            try:
                self.sock.send(data.encode())
            except:
                self.add_message("Ошибка отправки сообщения")
        ...
        if msg_type == "TEXT":
            ...
            if len(parts) >= 4:
                ...
                try:
                    ...
                    self.add_message(f"{author}: Отправил картинку: {filename}", img=ctk_img)
                except:
                    self.add_message("Ошибка приема картинки.")
    ...
    def open_img(self):
        ...
        try:
            ...
            self.add_message(f"Ошибка отправки картинки: {e}")
